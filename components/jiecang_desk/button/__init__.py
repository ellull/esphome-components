...
async def to_code(config):
    ...
    if CONF_POSITION_1 in config:
        btn = await button.new_button(config[CONF_POSITION_1])
        await cg.register_component(btn, config)
        cg.add(btn.set_parent(parent))
        cg.add(btn.set_command(0x05))

    if CONF_POSITION_2 in config:
        btn = await button.new_button(config[CONF_POSITION_2])
        await cg.register_component(btn, config)
        cg.add(btn.set_parent(parent))
        cg.add(btn.set_command(0x06))

    if CONF_POSITION_3 in config:
        btn = await button.new_button(config[CONF_POSITION_3])
        await cg.register_component(btn, config)
        cg.add(btn.set_parent(parent))
        cg.add(btn.set_command(0x25))

    if CONF_POSITION_4 in config:
        btn = await button.new_button(config[CONF_POSITION_4])
        await cg.register_component(btn, config)
        cg.add(btn.set_parent(parent))
        cg.add(btn.set_command(0x26))
